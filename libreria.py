...
class LinkedList:
    ...
    def devolver_libro(self, id_libro):
        libros_disponibles = []
        for libro in self:
            if libro.value.id_libro == id_libro and libro.value.alquilado:
                libro.value.alquilado = False
                while True:
                    try:
                        id_libro = str(input("Estado del libro? "))
                        if id_libro == "Bueno":
                        ...
                        for libro in libros_disponibles:
                         if libro.id_libro == id_libro:
                            libro.alquilado = True
                            print(f"Has devuelto el libro '{libro.value.titulo}'en buen estado ")
                        if  id_libro == "Malo":
                           print(f"Has devuelto el libro '{libro.value.titulo}'en mal estado ")
                           return
    ...
    def aplicar_descuento(self, libros_alquilados):
     ...
     if len(libros_alquilados) >= 2:
        total_tarifa_alquiler = sum(libro.tarifa_alquiler for libro in libros_alquilados)
        descuento_total = total_tarifa_alquiler * 0.2
        total_con_descuento = total_tarifa_alquiler - descuento_total

        print("Se aplicó un descuento del 20% a los libros alquilados:")
        for libro in libros_alquilados:
            print(f"Libro '{libro.titulo}': Tarifa de alquiler original: {libro.tarifa_alquiler}, Tarifa diaria con descuento: {libro.tarifa_alquiler - (libro.tarifa_alquiler * 0.2)}")
        print(f"Total a pagar con descuento: {total_con_descuento}")
        print("\n ")

     if len(libros_alquilados) >= 3:
        total_tarifa_alquiler = sum(libro.tarifa_alquiler for libro in libros_alquilados)
        descuento_total = total_tarifa_alquiler * 0.3
        total_con_descuento = total_tarifa_alquiler - descuento_total

        print("Se aplicó un descuento del 30% a los libros alquilados:")
        for libro in libros_alquilados:
            print(f"Libro '{libro.titulo}': Tarifa de alquiler original: {libro.tarifa_alquiler}, Tarifa diaria con descuento: {libro.tarifa_alquiler - (libro.tarifa_alquiler * 0.3)}")
        print(f"Total a pagar con descuento: {total_con_descuento}")
